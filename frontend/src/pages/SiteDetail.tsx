    ...
    const items = [
        {
            ...
            children: (
                <div className="space-y-6">
                    <Card title="场地基础信息" className="glass-panel border-slate-200 dark:border-slate-700/50" extra={<Button type="link" icon={<EditOutlined />}>编辑</Button>}>
                    ...
                    <Card title="结算规则配置" className="glass-panel border-slate-200 dark:border-slate-700/50">
                        <Descriptions column={2} className="text-slate-600 dark:text-slate-300">
        ...
        },
        {
            ...
            children: (
                <Card className="glass-panel border-slate-200 dark:border-slate-700/50" bodyStyle={{ padding: 0 }}>
                    <div className="p-4 border-b border-slate-200 dark:border-slate-700/50 flex justify-between">
                        <Space>
                    ...
                    <Table
                        ...
                        columns={[
                            ...
                            { title: '操作', key: 'action', render: () => <a className="text-red-600 dark:text-red-400">作废</a> }
                        ]}
                        pagination={false}
        ...
        },
        {
            ...
            children: (
                <div className="space-y-6">
                    <Card title="审批阶段资料" className="glass-panel border-slate-200 dark:border-slate-700/50" extra={<Button type="primary" size="small">上传资料</Button>}>
                        <List
                            dataSource={[
                            ...
                            renderItem={item => (
                                <List.Item actions={[<a>预览</a>, <a>下载</a>]}>
                                    <List.Item.Meta title={<span className="text-slate-900 dark:text-white">{item.name}</span>} description={<span className="text-slate-600 dark:text-slate-400">{item.uploader} 上传于 {item.time}</span>} />
                                </List.Item>
                            )}
                        />
                    </Card>
                    <Card title="运营阶段资料" className="glass-panel border-slate-200 dark:border-slate-700/50" extra={<Button type="primary" size="small">上传资料</Button>}>
                        <List
                            dataSource={[
        ...
        },
        {
            ...
            children: (
                <div className="space-y-6">
                    <Card title="设备配置" className="glass-panel border-slate-200 dark:border-slate-700/50" extra={<Button type="primary" size="small">新增设备</Button>}>
                        <List
                            dataSource={[
                            ...
                            renderItem={item => (
                                <List.Item actions={[<a>配置</a>]}>
                                    <List.Item.Meta
                                        title={<Space><span className="text-slate-900 dark:text-white">{item.name}</span><Tag color={item.status === '在线' ? 'success' : 'error'}>{item.status}</Tag></Space>}
                                        description={<span className="text-slate-600 dark:text-slate-400">类型: {item.type} | IP: {item.ip}</span>}
                                    />
                                </List.Item>
                            )}
                        />
                    ...
                    <Card title="运营配置" className="glass-panel border-slate-200 dark:border-slate-700/50">
                        <Descriptions column={1} labelStyle={{ width: '200px' }}>
                            <Descriptions.Item label="排号规则">
                                <Switch defaultChecked /> <span className="ml-2 text-slate-600 dark:text-slate-300">开启后车辆需排队入场</span>
                            </Descriptions.Item>
                            <Descriptions.Item label="最大等待数">
                                <InputNumber defaultValue={50} min={1} />
                            </Descriptions.Item>
                            <Descriptions.Item label="人工消纳开关">
                                <Switch /> <span className="ml-2 text-slate-600 dark:text-slate-300">开启后可在系统手动录入消纳记录(用于异常情况)</span>
                            </Descriptions.Item>
    ...
    return (
        ...
        >
            <div className="flex items-center gap-4 mb-6">
                ...
                />
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">{siteInfo.name}</h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">场地类型: {siteInfo.type}</p>
                </div>
            </div>
