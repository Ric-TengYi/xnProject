...
const SitesDisposals: React.FC = () => {
    const columns = [
        { title: '记录编号', dataIndex: 'id', key: 'id', render: (text: string) => <span className="font-mono text-slate-600 dark:text-slate-400">{text}</span> },
        { title: '场地名称', dataIndex: 'site', key: 'site', render: (text: string) => <span className="text-blue-600 dark:text-blue-400">{text}</span> },
        ...
        { title: '消纳量 (方)', dataIndex: 'volume', key: 'volume', render: (val: number) => <span className="text-green-600 dark:text-green-500 font-bold">{val}</span> },
        { 
            title: '状态', 
            dataIndex: 'status', 
            key: 'status',
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">全局消纳清单</h1>
            </div>
            ...
            <Card className="glass-panel border-slate-200 dark:border-slate-700/50">
                <div className="flex justify-between mb-4 flex-wrap gap-4">
                    <Space className="flex-wrap">
                        ...
                        <Input placeholder="搜索车牌/项目" prefix={<SearchOutlined />} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white w-48" />
                        <RangePicker className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700" showTime />
                    ...
                    <Button icon={<DownloadOutlined />} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white hover:text-blue-600 dark:text-blue-400 hover:border-blue-400">导出数据</Button>
                </div>
