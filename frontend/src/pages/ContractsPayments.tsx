...
const ContractsPayments: React.FC = () => {
    ...
    const columns = [
        { title: '合同编号', dataIndex: 'id', key: 'id', render: (text: string) => <span className="font-mono text-slate-600 dark:text-slate-300">{text}</span> },
        { title: '关联项目', dataIndex: 'project', key: 'project', render: (text: string) => <span className="text-blue-600 dark:text-blue-400">{text}</span> },
        ...
        { title: '已入账 (元)', dataIndex: 'paidAmount', key: 'paidAmount', render: (val: number) => <span className="text-green-600 dark:text-green-500">{val.toLocaleString()}</span> },
        { title: '待入账 (元)', dataIndex: 'unPaidAmount', key: 'unPaidAmount', render: (val: number) => <span className={val > 0 ? 'text-red-600 dark:text-red-500' : 'text-slate-600 dark:text-slate-400'}>{val.toLocaleString()}</span> },
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">合同入账管理</h1>
            </div>

            <Card className="glass-panel border-slate-200 dark:border-slate-700/50">
                <div className="flex justify-between mb-4">
                    <Space>
                        <Input placeholder="搜索合同编号/项目" prefix={<SearchOutlined />} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white w-64" />
            ...
            >
                <Form form={form} layout="vertical">
                    <Form.Item label="合同编号">
                        <Input disabled value={currentContract?.id} className="bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300" />
                    </Form.Item>
                    <Form.Item label="待入账金额">
                        <Input disabled value={`¥ ${currentContract?.unPaidAmount?.toLocaleString()}`} className="bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300" />
                    </Form.Item>
