...
const SitesBasicInfo: React.FC = () => {
    ...
    const columns = [
        { title: '场地名称', dataIndex: 'name', key: 'name', render: (text: string, record: any) => <a onClick={() => navigate(`/sites/${record.id}?tab=info`)} className="text-blue-600 dark:text-blue-400 font-medium">{text}</a> },
            ...
            render: (_: any, record: any) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-600 dark:text-slate-400 w-16">剩余: {((record.totalCapacity - record.usedCapacity) / 10000).toFixed(1)}万</span>
                    </div>
                );
            }
        },
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">场地基础信息</h1>
            </div>

            <Card className="glass-panel border-slate-200 dark:border-slate-700/50">
                <div className="flex justify-between mb-4">
                    <Space>
                        <Input placeholder="搜索场地名称" prefix={<SearchOutlined />} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white w-64" />
