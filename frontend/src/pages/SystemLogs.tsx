...
const SystemLogs: React.FC = () => {
    ...
    const loginColumns = [
        { title: '登录账号', dataIndex: 'account', key: 'account', render: (t: string) => <strong className="text-slate-700 dark:text-slate-200">{t}</strong> },
        { title: '登录时间', dataIndex: 'time', key: 'time', render: (t: string) => <span className="text-slate-600 dark:text-slate-400 font-mono">{t}</span> },
        { title: 'IP 地址', dataIndex: 'ip', key: 'ip', render: (t: string) => <span className="text-slate-600 dark:text-slate-400">{t}</span> },
        { title: '终端信息', key: 'device', render: (_: any, r: any) => <span className="text-slate-600 dark:text-slate-400 text-sm">{r.os} / {r.browser}</span> },
        { 
            title: '状态', 
            dataIndex: 'status', 
            key: 'status',
    ...
    const operateColumns = [
        { title: '操作人', dataIndex: 'operator', key: 'operator', render: (t: string) => <strong className="text-slate-700 dark:text-slate-200">{t}</strong> },
        { title: '操作模块', dataIndex: 'module', key: 'module', render: (t: string) => <Tag color="blue" className="border-none">{t}</Tag> },
        { title: '操作类型', dataIndex: 'action', key: 'action', render: (t: string) => <span className="text-slate-600 dark:text-slate-300">{t}</span> },
        { title: '操作内容', dataIndex: 'content', key: 'content', render: (t: string) => <span className="text-slate-600 dark:text-slate-400">{t}</span> },
        { title: '操作时间', dataIndex: 'time', key: 'time', render: (t: string) => <span className="text-slate-600 dark:text-slate-400 font-mono">{t}</span> },
        { title: 'IP 地址', dataIndex: 'ip', key: 'ip', render: (t: string) => <span className="text-slate-600 dark:text-slate-400 text-sm">{t}</span> },
    ];
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">系统日志</h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">查询系统登录日志、操作行为审计及系统错误日志</p>
                </div>
            </div>
                ...
                <Tabs defaultActiveKey="1" className="custom-tabs px-6 pt-4">
                    <TabPane tab="操作日志" key="1">
                        <div className="pb-4 flex flex-wrap gap-4">
                            <Input placeholder="搜索操作人/模块/内容" prefix={<SearchOutlined className="text-slate-600 dark:text-slate-400" />} className="w-64 bg-white dark:bg-slate-800/80 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white" value={searchText} onChange={e => setSearchText(e.target.value)} />
                    ...
                    <TabPane tab="登录日志" key="2">
                        <div className="pb-4 flex flex-wrap gap-4">
                            <Input placeholder="搜索账号/IP" prefix={<SearchOutlined className="text-slate-600 dark:text-slate-400" />} className="w-64 bg-white dark:bg-slate-800/80 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white" />
                            <RangePicker className="bg-white dark:bg-slate-800/80 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white" />
                            <div className="flex-1 flex justify-end">
                                <Button icon={<ExportOutlined />} className="bg-transparent text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:text-slate-900 dark:text-white">导出</Button>
                            </div>
