    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white m-0">项目数据看板</h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">实时展示各项目的工程量进度与当日消纳排名</p>
                </div>
                <div className="text-slate-600 dark:text-slate-400 text-sm">自动刷新: 开启</div>
            </div>
                ...
                {projectData.map((project, index) => {
                    ...
                    return (
                        <Col xs={24} sm={12} lg={8} key={project.id}>
                            ...
                            >
                                <Card 
                                    className="glass-panel hover:-translate-y-1 transition-transform cursor-pointer border-slate-200 dark:border-slate-700/50 hover:border-blue-500/50"
                                    onClick={() => navigate(`/projects/${project.id}`)}
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="w-2/3">
                                            <div className="flex items-center gap-2 mb-1">
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${index < 3 ? 'bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
                                                    {project.rank}
                                                </div>
                                                <h3 className="text-lg font-bold text-slate-900 dark:text-white m-0 truncate" title={project.name}>{project.name}</h3>
                                            </div>
                                            <div className="text-xs text-slate-600 dark:text-slate-400 mt-1 truncate pl-8">{project.builder}</div>
                                        </div>
                                        <Statistic 
                                            title={<span className="text-slate-600 dark:text-slate-400 text-xs">今日出土</span>}
                                            value={project.today} 
                                            suffix="方" 
                                    ...
                                    <div className="mt-6">
                                        <div className="flex justify-between text-xs mb-1">
                                            <span className="text-slate-600 dark:text-slate-400">项目总进度</span>
