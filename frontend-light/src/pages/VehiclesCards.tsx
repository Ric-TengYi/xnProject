...
const VehiclesCards: React.FC = () => {
    ...
    const columns = [
        { title: '卡号', dataIndex: 'id', key: 'id', render: (text: string) => <span className="font-mono g-text-secondary">{text}</span> },
        ...
        { title: '绑定车牌', dataIndex: 'plate', key: 'plate', render: (text: string) => text === '-' ? <span className="g-text-secondary">未绑定</span> : <Tag color="default">{text}</Tag> },
        { title: '当前余额 (元)', dataIndex: 'balance', key: 'balance', render: (val: number) => <span className={`font-bold ${val < 500 ? 'g-text-error' : 'g-text-success'}`}>{val.toFixed(2)}</span> },
        ...
        <div className="space-y-6">
            ...
            <Row gutter={[24, 24]}>
                <Col span={8}>
                    <Card className="glass-panel border-l-4 border-l-blue-500">
                        <Statistic title={<span className="g-text-secondary">发卡总数 (张)</span>} value={125} prefix={<CreditCardOutlined className="g-text-primary-link" />} valueStyle={{ color: 'var(--text-primary)' }} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card className="glass-panel border-l-4 border-l-orange-500">
                        <Statistic title={<span className="g-text-secondary">油卡总余额 (元)</span>} value={145800.50} precision={2} prefix={<WalletOutlined className="g-text-warning" />} valueStyle={{ color: 'var(--text-primary)' }} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card className="glass-panel border-l-4 border-l-green-500">
                        <Statistic title={<span className="g-text-secondary">电卡总余额 (元)</span>} value={89200.00} precision={2} prefix={<WalletOutlined className="g-text-success" />} valueStyle={{ color: 'var(--text-primary)' }} />
                    </Card>
                </Col>
            </Row>
            ...
            <Card className="glass-panel g-border-panel border">
                <div className="flex justify-between mb-4">
                    <Space>
                        <Input placeholder="搜索卡号/车牌" prefix={<SearchOutlined />} className="bg-white g-border-panel border g-text-primary w-64" />
            ...
            >
                <Form form={form} layout="vertical">
                    <Form.Item label="当前卡号">
                        <Input disabled value={currentCard?.id} className="bg-white g-text-secondary" />
                    </Form.Item>
                    <Form.Item label="当前余额">
                        <Input disabled value={`¥ ${currentCard?.balance?.toFixed(2)}`} className="bg-white g-text-secondary" />
                    </Form.Item>
