...
const EventsManagement: React.FC = () => {
    ...
    const columns = [
        { title: '事件编号', dataIndex: 'id', key: 'id', render: (t: string) => <span className="g-text-primary-link font-mono">{t}</span> },
        { title: '事件类型', dataIndex: 'type', key: 'type', render: (t: string) => <Tag color="blue" className="border-none">{t}</Tag> },
        { title: '标题', dataIndex: 'title', key: 'title', render: (t: string) => <strong className="g-text-primary">{t}</strong> },
        { title: '申报人', dataIndex: 'reporter', key: 'reporter', render: (t: string) => <span className="g-text-secondary">{t}</span> },
        { title: '申报时间', dataIndex: 'time', key: 'time', render: (t: string) => <span className="g-text-secondary">{t}</span> },
        { 
            title: '状态', 
            dataIndex: 'status', 
            key: 'status',
        ...
        { 
            ...
            render: (_: any, record: any) => (
                <a className="g-text-primary-link hover:g-text-primary-link" onClick={() => handleView(record)}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="g-text-secondary mt-1">管理人工上报的异常事件，支持审核流转与闭环处理</p>
                </div>
            </div>

            <Card className="glass-panel g-border-panel border" bodyStyle={{ padding: 0 }}>
                <div className="p-4 border-b g-border-panel border flex gap-4 g-bg-toolbar">
                    <Input 
            ...
            <Drawer
                title={<span className="g-text-primary">事件详情</span>}
                placement="right"
                width={500}
            ...
            >
                {selectedEvent && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <div className="g-text-secondary mb-2">事件标题</div>
                            <div className="g-text-primary text-lg font-bold">{selectedEvent.title}</div>
                        </div>
                        
                        <div>
                            <div className="g-text-secondary mb-2">详细描述</div>
                            <div className="g-text-secondary bg-white p-4 rounded-lg border g-border-panel border min-h-[100px]">
