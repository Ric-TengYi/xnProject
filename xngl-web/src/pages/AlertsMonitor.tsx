    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold g-text-primary m-0 flex items-center gap-2">
                        <ShieldAlert className="g-text-error" />
                        预警与监控中心
                    </h1>
                    <p className="g-text-secondary mt-1">依托违规车辆研判模型，对违法事件实时感知与阻断</p>
                </div>
                <div className="flex gap-4 items-center">
                    <Badge status="processing" text={<span className="g-text-success">研判模型运行中</span>} />
                    <Select
                        defaultValue="全部"
                        style={{ width: 120 }}
            ...
            <Row gutter={[24, 24]}>
                <Col span={16}>
                    <Card
                        className="glass-panel min-h-[500px] g-border-panel border"
                        bodyStyle={{ padding: 0 }}
                    >
                        <Tabs defaultActiveKey="1" className="custom-tabs px-6 pt-4">
                                ...
                                <List
                                    ...
                                    renderItem={(item) => (
                                        <AnimatePresence>
                                            ...
                                            >
                                                <List.Item className="border-b g-border-panel border py-4 hover:bg-white/30 px-2 rounded transition-colors group">
                                                    <List.Item.Meta
                                                        avatar={
                                                            <div className="relative">
                                                                <Avatar size="large" icon={<Truck size={20} />} className="bg-white border border-slate-600 g-text-secondary" />
                                                                <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-white" style={{ backgroundColor: getLevelColor(item.level) }}></div>
                                                            </div>
                                                        }
                                                        title={
                                                            <div className="flex justify-between items-center w-full">
                                                                <div className="flex items-center gap-3">
                                                                    <span className="g-text-primary font-bold text-lg">{item.vehicle}</span>
                                                                    <Tag color={getLevelColor(item.level)} className="border-none font-bold">
                                                                        {item.type}
                                                                    </Tag>
                                                                    <Tag color={item.status === '待处置' ? 'magenta' : item.status === '处置中' ? 'orange' : 'default'} className="border-slate-600 bg-transparent g-text-secondary">
                                                                        {item.status}
                                                                    </Tag>
                                                                </div>
                                                                <span className="g-text-secondary text-sm flex items-center gap-1">
                                                                    <Clock size={14} /> {item.time}
                                                                </span>
                                                            </div>
                                                        }
                                                        description={
                                                            <div className="mt-2 g-text-secondary flex items-center justify-between">
                                                                <span className="flex items-center gap-1"><MapPin size={14} /> 所属项目: <span className="g-text-secondary">{item.project}</span></span>
                                                                {item.status === '待处置' && (
                            ...
                            <TabPane tab="场地预警" key="2">
                                <div className="p-4 g-text-secondary text-center">暂无场地预警数据</div>
                            </TabPane>
                            <TabPane tab="项目预警" key="3">
                                <div className="p-4 g-text-secondary text-center">暂无项目预警数据</div>
                            </TabPane>
                        </Tabs>
                    </Card>
                </Col>
                ...
                <Col span={8}>
                    <div className="space-y-6">
                        <Card
                            title={<span className="g-text-primary">高风险车队 TOP 3</span>}
                        ...
                        >
                            {[
                                ...
                                <div key={idx} className="flex justify-between items-center mb-4 last:mb-0">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-red-500/20 g-text-error' : idx === 1 ? 'bg-orange-500/20 g-text-warning' : 'bg-black/10 g-text-secondary'}`}>
                                            {idx + 1}
                                        </div>
                                        <span className="g-text-secondary">{team.name}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="g-text-secondary text-sm">{team.count} 起</span>
                                        <span className={team.up ? 'g-text-error' : 'g-text-success'}>{team.up ? '↑' : '↓'}</span>
                                    </div>
                                </div>
                            ))}
                        </Card>
                        ...
                        <Card
                            title={<span className="g-text-primary">动态电子围栏状态</span>}
                            className="glass-panel g-border-panel border"
                            headStyle={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}
                        >
                            ...
                            <div className="mt-4 pt-4 border-t g-border-panel border flex justify-between">
                                <span className="g-text-secondary text-sm">当前布控区域：12 个</span>
                                <a className="g-text-primary-link text-sm hover:g-text-primary-link">去配置</a>
                            </div>
                        </Card>
                    </div>
                </Col>
