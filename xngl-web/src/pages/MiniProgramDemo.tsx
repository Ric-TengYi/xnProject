...
const MiniProgramDemo: React.FC = () => {
  ...
  const [delayForm] = Form.useForm<{ projectId?: number; siteId?: number; requestedEndTime?: Dayjs; reason?: string; attachmentUrls?: string }>();
  ...
  const [delayApplies, setDelayApplies] = useState<MiniDelayApplyRecord[]>([]);
  ...
  useEffect(() => {
    delayForm.setFieldsValue({
      projectId: currentProjectId || undefined,
      siteId: currentSiteId || undefined,
      requestedEndTime: dayjs().add(2, 'day').hour(18).minute(0).second(0),
      reason: '受天气与场地调度影响，申请顺延移动端作业截止时间。',
      attachmentUrls: 'https://example.com/demo/delay-apply-proof.jpg',
    });
  }, [currentProjectId, currentSiteId, delayForm]);
  ...
  const reloadMiniWorkspace = async (token: string) => {
    ...
    const [photoRecords, disposalRecords, eventRecords, feedbackRecords, delayRecords] = await Promise.all([
  ...
  const handleCreateDelayApply = async () => {
    if (!miniToken) {
      message.warning('请先完成移动端登录');
      return;
    }
    const values = await delayForm.validateFields();
    await withSubmitting('delay-apply', async () => {
      await createMiniDelayApply(miniToken, {
        bizType: 'PROJECT',
        projectId: values.projectId ? Number(values.projectId) : undefined,
        siteId: values.siteId ? Number(values.siteId) : undefined,
        requestedEndTime: values.requestedEndTime
          ? values.requestedEndTime.format('YYYY-MM-DDTHH:mm:ss')
                  ...
                  },
                  {
                    ...
                    children: (
                      <div className="space-y-4 p-4">
                        ...
                        <Card size="small" title="消纳确认 / 手动消纳" extra={<TruckOutlined />}>
                        ...
                        <Card size="small" title="延期申报" extra={<NotificationOutlined />}>
                          <Form form={delayForm} layout="vertical">
                            ...
                            <Form.Item name="requestedEndTime" label="申请延期到" rules={[{ required: true, message: '请选择延期时间' }]}>
                              <DatePicker showTime className="w-full" />
                            </Form.Item>
                            <Form.Item name="reason" label="延期原因" rules={[{ required: true, message: '请输入延期原因' }]}>
                              <Input.TextArea rows={2} />
                            </Form.Item>
                            <Form.Item name="attachmentUrls" label="佐证附件地址">
                              <Input placeholder="可选，填写附件地址用于演示" />
                            </Form.Item>
                            <Button type="primary" block loading={submitting === 'delay-apply'} onClick={() => void handleCreateDelayApply()}>
                              提交延期申报
                            </Button>
                          </Form>
                        </Card>
        ...
        <Col xs={24} xl={14}>
          <div className="space-y-6">
            <Row gutter={[16, 16]}>
              ...
              <Col xs={12} md={6}>
                <Card className="glass-panel g-border-panel border">
                  <Statistic title="延期申报数" value={delayApplies.length} />
                </Card>
              </Col>
            </Row>
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <Card title="最近出土打卡 / 消纳清单" className="glass-panel g-border-panel border">
              ...
              <Col xs={24} lg={12}>
                <Card title="最近事件 / 延期 / 反馈" className="glass-panel g-border-panel border">
                  ...
                  />
                  <Divider />
                  <Title level={5}>延期申报</Title>
                  <List
                    dataSource={delayApplies.slice(0, 3)}
                    locale={{ emptyText: <Empty description="暂无延期申报记录" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }}
                    renderItem={(item) => (
                      <List.Item>
                        <List.Item.Meta
                          title={`${item.projectName || '-'} / ${item.siteName || '-'}`}
                          description={`${item.requestedEndTime || '-'} · ${item.reason || '-'}`}
                        />
                        <Tag color={item.status === 'SUBMITTED' ? 'processing' : 'success'}>
