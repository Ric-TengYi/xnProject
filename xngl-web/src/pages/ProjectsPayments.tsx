...
const ProjectsPayments: React.FC = () => {
    ...
    const columns = [
        ...
        { title: '累计交款 (元)', dataIndex: 'paidAmount', key: 'paidAmount', render: (val: number) => <span className="g-text-success">{val.toLocaleString()}</span> },
        { title: '欠款 (元)', dataIndex: 'debtAmount', key: 'debtAmount', render: (val: number) => <span className={val > 0 ? 'g-text-error' : 'g-text-secondary'}>{val.toLocaleString()}</span> },
        ...
        <div className="space-y-6">
            ...
            <Card className="glass-panel g-border-panel border">
                <div className="flex justify-between mb-4">
                    <Space>
                        <Input placeholder="搜索项目名称" prefix={<SearchOutlined />} className="bg-white g-border-panel border g-text-primary w-64" />
