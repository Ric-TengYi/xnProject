...
const buildSiteMetaText = (site: SiteViewModel) => {
  const items: string[] = [];
  if (site.parentSiteName) {
    items.push(`上级场地: ${site.parentSiteName}`);
  } else {
    items.push('直属场地');
  }
  if (site.weighbridgeSiteName) {
    items.push(`借用地磅: ${site.weighbridgeSiteName}`);
  }
  return items.join(' · ');
};
                ...
                {filteredSites.length === 0 ? (
                    ...
                    <Row gutter={[24, 24]}>
                        {filteredSites.map((site, index) => {
                            ...
                            const siteMetaText = buildSiteMetaText(site);
                            return (
                                <Col xs={24} sm={12} lg={8} xl={6} key={site.id} className="flex">
                                    <motion.div
                                    ...
                                    >
                                        <Card
                                            className="glass-panel h-full hover:-translate-y-1 transition-transform duration-300 g-border-panel border hover:border-blue-500/50 group"
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex items-center gap-2 min-w-0">
                                                        <Tag color="geekblue" className="w-fit m-0 border-none bg-opacity-20 shrink-0">{site.siteLevel}</Tag>
                                                        <span className="min-w-0 truncate text-xs g-text-secondary" title={siteMetaText}>
                                                            {siteMetaText}
                                                        </span>
                                                    </div>
                                            ...
                                            <div className="mt-auto flex gap-4 pt-4 border-t g-border-panel border">
